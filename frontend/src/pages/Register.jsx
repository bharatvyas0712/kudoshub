...
export default function Register() {
	...
	const onSubmit = async (values) => {
		...
		try {
			const sessionUser = await registerUser(values);
			navigate(sessionUser.role === 'ADMIN' ? '/admin' : '/dashboard', { replace: true });
		} catch (error) {
			setServerError(getErrorMessage(error, 'Unable to register'));
		}
	};
	...
	return (
		<div className="mx-auto grid min-h-[calc(100vh-4rem)] max-w-6xl place-items-center px-6 py-10">
			...
			>
				<div>
					<p className="text-sm uppercase tracking-[0.3em] text-cyan-400">Register</p>
